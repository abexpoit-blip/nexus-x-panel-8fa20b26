...
// ============================================================================
// Range Pool Grid — visual cards with stats + customization
// ============================================================================
...
const TAG_COLORS = [
  { key: "cyan",   cls: "from-neon-cyan/20 to-neon-cyan/5 border-neon-cyan/40 text-neon-cyan" },
  { key: "green",  cls: "from-neon-green/20 to-neon-green/5 border-neon-green/40 text-neon-green" },
  { key: "amber",  cls: "from-neon-amber/20 to-neon-amber/5 border-neon-amber/40 text-neon-amber" },
  { key: "purple", cls: "from-neon-purple/20 to-neon-purple/5 border-neon-purple/40 text-neon-purple" },
  { key: "pink",   cls: "from-pink-500/20 to-pink-500/5 border-pink-500/40 text-pink-400" },
  { key: "blue",   cls: "from-blue-500/20 to-blue-500/5 border-blue-500/40 text-blue-400" },
];
const colorClass = (key: string | null | undefined) =>
  TAG_COLORS.find(c => c.key === key)?.cls || "from-white/[0.04] to-white/[0.01] border-white/[0.08] text-foreground";

const RangeCard = ({ r, onChanged }: { r: RangeRow; onChanged: () => void }) => {
  ...
  const save = async () => {
    setSaving(true);
    try {
      await api.admin.numpanelRangeMetaSave({
        range_prefix: r.name,
  ...
  const reset = async () => {
    if (!confirm(`Reset customization for ${r.name}?`)) return;
    try {
      await api.admin.numpanelRangeMetaDelete(r.name);
      toast.success("Reset");
      onChanged();
      ...
      <div className="relative">
        <div className="flex items-start justify-between gap-2 mb-3">
          ...
          <button
            ...
            className="shrink-0 p-1 rounded hover:bg-white/10 text-muted-foreground hover:text-foreground transition"
            title="Customize"
          >
        ...
        <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
          ...
          {r.request_override != null && (
            <div className="flex items-center gap-1">
              <Hash className="w-3 h-3" />
              <span className="font-mono">×{r.request_override}</span>
            </div>
          )}
        </div>
            ...
            <div className="flex items-center gap-1 flex-wrap">
              ...
              {TAG_COLORS.map(c => (
                <button
                  ...
                  className={cn(
                    "w-5 h-5 rounded-full bg-gradient-to-br border-2 transition",
                    c.cls,
...
const RangePoolGrid = ({
  ranges, totalActive, totalUsed, onChanged,
}: { ranges: RangeRow[]; totalActive: number; totalUsed: number; onChanged: () => void }) => {
    ...
    <div className="glass-card border border-white/[0.06] rounded-xl p-5 space-y-4">
      ...
      {filtered.length ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 max-h-[600px] overflow-y-auto scrollbar-none pr-1">
          {filtered.map(r => (
            <RangeCard key={r.name} r={r} onChanged={onChanged} />
          ))}
        </div>
      ) : (
