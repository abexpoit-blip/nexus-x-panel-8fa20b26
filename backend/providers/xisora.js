// XISORA provider — pure HTTP cookie-session bot, mirrors IMS provider semantics.
// Pool is filled by xisoraBot via dt_numbers.php scrape; OTPs delivered via dt_reports.php.
...
module.exports = {
  ...
  async listCountries() {
    return db.prepare(`
      ...
      FROM rates WHERE provider = 'xisora' AND country_code IS NOT NULL
    `).all();
  },
  ...
  async listOperators() {
    return db.prepare(`
      ...
      WHERE provider = 'xisora' AND operator IS NOT NULL
    `).all();
  },
  ...
  async getNumber({ range, countryCode, operator } = {}) {
    let bot = null;
    try { bot = require('../workers/xisoraBot'); } catch (_) {}
    ...
    if (!row) {
      throw new Error(range
        ? `Range "${range}" has no fresh numbers (skipped ${skipped} stale, lost ${lost} race)`
        : 'No fresh XISORA numbers available');
    }
    if (skipped > 0 || lost > 0) console.log(`[xisora-provider] assigned ${row.phone_number} (skipped=${skipped} stale, lost=${lost})`);
    return {
