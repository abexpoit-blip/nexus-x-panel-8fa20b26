// SEVEN1TEL Browser Bot — headless Chrome that stays logged into 94.23.120.156/ints
// and scrapes the agent's numbers + OTP CDRs.
//
// SEVEN1TEL panel structure (verified live 2026-04-20):
...
//   SEVEN1TEL_ENABLED=true
//   SEVEN1TEL_BASE_URL=http://94.23.120.156
//   SEVEN1TEL_USERNAME=Sayedahmed
//   SEVEN1TEL_PASSWORD=Rumon1275
//   SEVEN1TEL_CHROME_PATH=/usr/bin/chromium-browser   (or empty for puppeteer's bundled chrome)
//   SEVEN1TEL_HEADLESS=true
//   SEVEN1TEL_SCRAPE_INTERVAL=5      (OTP poll interval seconds — min 3)
//   SEVEN1TEL_NUMBERS_INTERVAL=600   (number-pool refresh seconds — min 60, default 10min)
...
function normalizeBase(raw) {
  const fallback = 'http://94.23.120.156';
...
function resolveCreds() {
  ...
  const dbUser = readSetting('seven1tel_username');
  const dbPass = readSetting('seven1tel_password');
  const dbBase = readSetting('seven1tel_base_url');
  return {
    ENABLED: (dbEnabled !== null ? dbEnabled : (process.env.SEVEN1TEL_ENABLED || 'false')).toString().toLowerCase() === 'true',
    BASE_URL: normalizeBase(dbBase || process.env.SEVEN1TEL_BASE_URL),
    USERNAME: dbUser || process.env.SEVEN1TEL_USERNAME || '',
    PASSWORD: dbPass || process.env.SEVEN1TEL_PASSWORD || '',
  };
}
function resolveOtpInterval() {
  const db = +(readSetting('seven1tel_otp_interval') || 0);
  const env = +(process.env.SEVEN1TEL_SCRAPE_INTERVAL || 5);
  return Math.max(3, db > 0 ? db : env);
}
// One-time cleanup: if DB has a polluted seven1tel_base_url (e.g. saved with /ints/login),
// rewrite it to the normalized scheme+host so we don't keep building broken URLs.
try {
  const cur = readSetting('seven1tel_base_url');
  if (cur) {
    const fixed = normalizeBase(cur);
    if (fixed && fixed !== cur) {
      db.prepare(`
        INSERT INTO settings (key, value, updated_at) VALUES ('seven1tel_base_url', ?, strftime('%s','now'))
        ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = strftime('%s','now')
      `).run(fixed);
      console.log(`[seven1tel-bot] auto-cleaned seven1tel_base_url: "${cur}" → "${fixed}"`);
    }
  }
} catch (_) {}

let { ENABLED, BASE_URL, USERNAME, PASSWORD } = resolveCreds();
const HEADLESS = String(process.env.SEVEN1TEL_HEADLESS || 'true').toLowerCase() !== 'false';
const CHROME_PATH = process.env.SEVEN1TEL_CHROME_PATH || undefined;
let OTP_INTERVAL = resolveOtpInterval();
const NUMBERS_INTERVAL = Math.max(60, +(process.env.SEVEN1TEL_NUMBERS_INTERVAL || 600));
const EMPTY_LIMIT = Math.max(0, +(process.env.SEVEN1TEL_EMPTY_LIMIT || 0)); // 0 = disabled by default for SEVEN1TEL
...
// Cookie domain — SEVEN1TEL runs on bare IP so we strip protocol
function cookieDomain() {
  try { return new URL(BASE_URL).hostname; } catch (_) { return '94.23.120.156'; }
}
...
function getStatus() {
  return db.bestEffortRead('seven1tel status counts', () => {
    const poolSize = db.prepare("SELECT COUNT(*) c FROM allocations WHERE provider='seven1tel' AND status='pool'").get().c;
    const claimingSize = db.prepare("SELECT COUNT(*) c FROM allocations WHERE provider='seven1tel' AND status='claiming'").get().c;
    const activeAssigned = db.prepare("SELECT COUNT(*) c FROM allocations WHERE provider='seven1tel' AND status='active'").get().c;
    const otpReceived = db.prepare("SELECT COUNT(*) c FROM allocations WHERE provider='seven1tel' AND status='received'").get().c;
    const hasCookies = !!readSetting('seven1tel_cookies');
    return {
...
async function loginOnce() {
  dlog('[seven1tel-bot] navigating to login page');
  // Block recaptcha + heavy assets so page actually finishes loading in headless
  try {
    if (!page.__seven1telReqIntercept) {
      ...
      page.__seven1telReqIntercept = true;
    }
  } catch (_) {}
  ...
  // Resolve username/password fields (SEVEN1TEL uses unnamed inputs inside a form)
  ...
  if (fields.captchaText && fields.captchaSel) {
    ...
    if (answer) {
      dlog(`[seven1tel-bot] captcha "${fields.captchaText.trim()}" → ${answer}`);
      await page.click(fields.captchaSel, { clickCount: 3 }).catch(() => {});
      await page.type(fields.captchaSel, answer, { delay: 25 });
    } else {
      dwarn('[seven1tel-bot] captcha detected but could not solve:', fields.captchaText);
    }
  }
  ...
  dlog(`[seven1tel-bot] login ${ok ? '✓' : '✗'} (url=${url})`);
  if (!ok) throw new Error('SEVEN1TEL login failed (likely captcha)');
}
...
async function tryCookieAuth() {
  const raw = readSetting('seven1tel_cookies');
  if (!raw) return false;
  const cookies = parseCookies(raw);
  if (!cookies.length) {
    dwarn('[seven1tel-bot] saved cookies present but parse returned 0 entries');
    return false;
  }
  try {
    ...
    const url = page.url();
    if (/\/login/i.test(url)) {
      dlog('[seven1tel-bot] cookie auth: redirected to /login → expired');
      _cookieFailStreak++;
    ...
    if (!hasContent) {
      dlog('[seven1tel-bot] cookie auth: no logged-in content');
      _cookieFailStreak++;
    ...
    _cdrReady = true;
    _cookieFailStreak = 0;
    console.log('[seven1tel-bot] ✓ logged in via saved cookies (skipped captcha)');
    logEvent('success', 'Logged in via saved session cookies (no captcha needed)');
    return true;
  } catch (e) {
    dwarn('[seven1tel-bot] cookie auth failed:', e.message);
    _cookieFailStreak++;
...
function maybeAlertCookieExpired(reason) {
  ...
  logEvent('error', `SEVEN1TEL cookies expired (${_cookieFailStreak} consecutive fails) — refresh needed`);
}
    ...
    try {
      ...
      try {
        ...
        if (fresh && fresh.length) {
          ...
          dlog(`[seven1tel-bot] saved ${fresh.length} fresh cookies for next session`);
          logEvent('success', `Saved ${fresh.length} fresh session cookies`);
        }
      } catch (e) { dwarn('[seven1tel-bot] failed to save fresh cookies:', e.message); }
      return;
    } catch (e) {
      lastErr = e;
      console.warn(`[seven1tel-bot] login attempt ${attempt}/3 failed: ${e.message}`);
      logEvent('warn', `Login attempt ${attempt}/3 failed: ${e.message}`);
      await new Promise(r => setTimeout(r, 1500 * attempt));
    }
  }
  throw lastErr || new Error('SEVEN1TEL login failed after 3 attempts');
}

// ---- Scrape MySMSNumbers (number pool) ----
// SEVEN1TEL table layout (verified live): Range | Prefix | Number | My Payout | Client | Payout | Limits
// We must:
...
async function scrapeNumbers() {
  ...
  // SEVEN1TEL panel pre-selects the first range by default, hiding all other ranges.
  await page.evaluate(() => {
  ...
  // Also click the Filter button if present (SEVEN1TEL requires clicking Filter after dropdown change)
  await page.evaluate(() => {
  ...
  dlog(`[seven1tel-bot] scrapeNumbers init: page-size selectors bumped=${sizeInfo.bumped} info="${sizeInfo.info}" rows=${sizeInfo.tableRows}`);
  ...
  const extractRows = () => page.evaluate(() => {
    ...
    document.querySelectorAll('table tbody tr').forEach((row) => {
      ...
      // SEVEN1TEL: cells = [chk, Range, Prefix, Number, MyPayout, Client, Payout, Limits]
      // Find the actual SEVEN1TELSDN: 10-15 digits, NOT the prefix (prefix is shorter ≤8 typically)
  ...
  dlog(`[seven1tel-bot] page 1: ${all.length} unique numbers so far`);

  // Paginate up to 500 pages (with 100/page that's 50k numbers — way more than SEVEN1TEL accounts hold)
  for (let i = 0; i < 500; i++) {
    ...
    if (all.length === before) {
      ...
      dlog(`[seven1tel-bot] pagination stopped at page ${i + 2} (no new rows)`);
      break;
    }
  }
  dlog(`[seven1tel-bot] scrapeNumbers DONE: ${all.length} total unique numbers across all pages`);
  return all;
}

// ---- Scrape SMS CDR Reports (OTPs) ----
// SEVEN1TEL auto-loads with TODAY's date range. We just reload for fresh data.
let _cdrReady = false;
...
async function deliverOtps() {
  const otps = await scrapeOtps().catch((e) => { dwarn('[seven1tel-bot] scrapeOtps:', e.message); return []; });
  ...
  const active = db.prepare(`
    ...
    WHERE provider='seven1tel' AND status='active' AND otp IS NULL
  `).all();
  ...
  for (const a of active) {
    ...
    if (cached.date_ts && cached.date_ts < allocAt - 120) {
      ...
      const dup = db.prepare(`
        ...
        WHERE provider='seven1tel' AND phone_number=? AND otp=? AND id<>?
        LIMIT 1
    ...
    try {
      ...
      delivered++;
      console.log(`[seven1tel-bot] OTP delivered: ${a.phone_number} → ${cached.otp_code} (alloc#${a.id})`);
      logEvent('success', `OTP delivered to ${a.phone_number}`, { otp: cached.otp_code, alloc: a.id });
    } catch (e) {
      dwarn(`[seven1tel-bot] markOtpReceived failed for ${a.phone_number}:`, e.message);
    }
  }
  return delivered;
}
...
function ensurePoolUser() {
  let u = db.prepare("SELECT id FROM users WHERE username = '__seven1tel_pool__'").get();
  if (!u) {
...
async function syncPool() {
  ...
  const exists = db.prepare("SELECT 1 FROM allocations WHERE provider='seven1tel' AND phone_number=? LIMIT 1");
  ...
  `);
  const poolRows = db.prepare("SELECT id, phone_number FROM allocations WHERE provider='seven1tel' AND status='pool'").all();
...
function start() {
  ...
  if (!ENABLED) {
    ...
    status.lastError = 'SEVEN1TEL bot disabled';
    status.lastErrorAt = now;
    logEvent('warn', 'Start skipped — bot disabled');
    console.log('✗ SEVEN1TEL bot disabled (set SEVEN1TEL_ENABLED=true or enable from admin panel)');
    return false;
  }
  if (!USERNAME || !PASSWORD) {
    console.warn('✗ SEVEN1TEL bot: credentials not set');
    status.lastError = 'SEVEN1TEL credentials not set';
    status.lastErrorAt = now;
    status.running = false;
    status.loggedIn = false;
    logEvent('error', 'Start skipped — SEVEN1TEL credentials not set');
    return false;
  }
  ...
  try {
    const r = db.prepare("UPDATE allocations SET status='pool' WHERE provider='seven1tel' AND status='claiming'").run();
    if (r.changes) console.log(`[seven1tel-bot] recovered ${r.changes} 'claiming' allocations → 'pool'`);
  } catch (_) {}
  ...
  _stopped = false;
  console.log(`✓ SEVEN1TEL bot starting (OTP poll ${OTP_INTERVAL}s, number sync ${NUMBERS_INTERVAL}s, headless=${HEADLESS}, base=${BASE_URL})`);
  ...
  setTimeout(async () => {
    try {
      await ensureBrowser();
      if (!loggedIn) await login();
      console.log('[seven1tel-bot] initial login complete — running first pool sync');
      await syncPool().catch(e => console.warn('[seven1tel-bot] initial sync failed:', e.message));
    } catch (e) {
      console.error('[seven1tel-bot] initial login failed:', e.message);
  ...
  function scheduleOtp() {
    ...
    otpTimer = setTimeout(async () => {
      ...
      try {
        ...
        status.consecFail = 0;
        if (delivered > 0) console.log(`[seven1tel-bot] poll delivered ${delivered} OTP(s) in ${Date.now() - t0}ms`);
      } catch (e) {
        status.consecFail++;
        status.lastError = e.message;
        status.lastErrorAt = Math.floor(Date.now() / 1000);
        status.lastScrapeOk = false;
        console.warn(`[seven1tel-bot] otp-poll fail #${status.consecFail}:`, e.message);
        logEvent('warn', `OTP poll failed (#${status.consecFail}): ${e.message}`);
        // Recycle browser after 3 consecutive fails
        if (status.consecFail >= 3) {
          console.warn('[seven1tel-bot] 3 consecutive fails — recycling browser');
  ...
  numbersTimer = setInterval(async () => {
    ...
    busy = true;
    try { await syncPool(); }
    catch (e) { console.warn('[seven1tel-bot] periodic syncPool failed:', e.message); }
    finally { busy = false; }
