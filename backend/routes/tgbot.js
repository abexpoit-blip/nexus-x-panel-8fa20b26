...
router.get('/config', (req, res) => {
  const keys = ['tg_public_channel', 'tg_otp_feed_chat', 'tg_required_group', 'tg_required_group_chat', 'tg_required_otp_group', 'tg_required_otp_group_chat', 'tg_terms_text', 'tg_billing_enabled'];
  ...
  res.json({
    ...
    tg_otp_feed_chat: settings.tg_otp_feed_chat || '',
    ...
    tg_billing_enabled: settings.tg_billing_enabled == null ? '1' : settings.tg_billing_enabled,
  });
});

router.put('/config', (req, res) => {
  const payload = req.body || {};
  const allowed = ['tg_public_channel', 'tg_otp_feed_chat', 'tg_required_group', 'tg_required_group_chat', 'tg_required_otp_group', 'tg_required_otp_group_chat', 'tg_terms_text', 'tg_billing_enabled'];
  const stmt = db.prepare(`
  ...
  db.transaction(() => {
    for (const key of allowed) {
      if (key in payload) stmt.run(key, String(payload[key] || '').trim());
    }
  })();
  logFromReq(req, 'tgbot_config_update', { meta: payload });
  res.json({ ok: true });
});
